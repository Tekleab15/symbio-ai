...
# Safe defaults before importing router.py file
...
# Gemma 4 cascade preserved for audit/partner for utilization of private on-demand deployment
...
os.environ.setdefault("SYMBIO_MAX_CONCURRENCY", "6")
os.environ.setdefault("SYMBIO_MAX_BATCH_SIZE", "512")
os.environ.setdefault("SYMBIO_SANDBOX_TIMEOUT_SECONDS", "2.0")
os.environ.setdefault("SYMBIO_REPLACE_CODEGEN_WITH_STDOUT", "0")
...
class ResultItem(BaseModel):
    ...
    source: str = Field(..., description="Route source: deterministic, fireworks, deterministic_repair, etc.")
...
_STDOUT_INTENT_PHRASES = (
    ...
    "execute the code", "calculate", "compute", "evaluate",
    "solve", "final numeric answer",
)
...
async def _postprocess_with_sandbox(*, task: Any, route_result: RouteResult) -> Tuple[str, str]:
    ...
    try:
        success, stdout, stderr = await asyncio.to_thread(run_sandboxed_python, raw_answer, timeout_seconds)
    ...
    if _should_replace_with_stdout(task_type=task_type, task_promptish=_task_to_promptish_text(task), stdout=stdout):
