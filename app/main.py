...
# Safe defaults before importing router.py

os.environ.setdefault("FIREWORKS_BASE_URL", "https://api.fireworks.ai/inference/v1")

# Serverless-safe defaults for live evaluator reliability.
os.environ.setdefault("FIREWORKS_MODEL_CHEAP", "accounts/fireworks/models/minimax-m3")
os.environ.setdefault("FIREWORKS_MODEL_FACTUAL", "accounts/fireworks/models/minimax-m3")
os.environ.setdefault("FIREWORKS_MODEL_CODE", "accounts/fireworks/models/kimi-k2p7-code")

# Gemma 4 cascade preserved for audit/partner prize and private on-demand deployment override.
os.environ.setdefault("FIREWORKS_MODEL_GEMMA", "accounts/fireworks/models/gemma-4-26b-a4b-it")
os.environ.setdefault(
    "FIREWORKS_MODEL_FALLBACKS",
    ",".join(
        [
...
os.environ.setdefault("SYMBIO_DISABLE_REASONING_EFFORT", "1")
os.environ.setdefault("SYMBIO_ENABLE_CACHE", "0")
os.environ.setdefault("SYMBIO_TIMEOUT_SECONDS", "90")
...
class ResultItem(BaseModel):
    ...
    source: str = Field(..., description="Route source: deterministic, fireworks, fallback_error, etc.")
    id: Optional[Any] = Field(default=None, description="Preserved task id if provided.")
...
async def lifespan(app_: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info(
        "symbioAI router initialized | base_url=%s | cheap=%s | factual=%s | code=%s | gemma=%s",
        ...
        os.getenv("FIREWORKS_MODEL_GEMMA"),
    )
...
def _extract_task_id(task: Any) -> Optional[Any]:
    if not isinstance(task, dict):
        return None

    for key in ("id", "task_id", "uid", "uuid"):
        if key in task:
            return task[key]
...
_STDOUT_INTENT_PHRASES = (
    ...
    "execute the code", "calculate", "compute",
    "evaluate", "solve", "final numeric answer",
)
...
async def _postprocess_with_sandbox(
    *,
    ...
    final_answer = canonicalize_answer(raw_answer, task_type, prompt)
    final_source = source
    ...
    if _should_replace_with_stdout(
        ...
        return canonicalize_answer(stdout, stdout_type, prompt), f"{source}+sandbox_stdout"
...
async def _process_one_task(task: Any) -> ResultItem:
    ...
    try:
        ...
        prompt = extract_prompt(task)
        answer = canonicalize_answer(answer, route_result.task_type, prompt)
...
async def health() -> HealthResponse:
    ...
    return HealthResponse(
        ...
        gemma_model=os.getenv("FIREWORKS_MODEL_GEMMA", ""),
        cache_enabled=os.getenv("SYMBIO_ENABLE_CACHE", "0").strip() == "1",
    )

@app.post("/process", response_model=ProcessResponse, response_model_exclude_none=True)
...
@app.post("/predict", response_model=ProcessResponse, response_model_exclude_none=True)
async def predict_alias(request: Request) -> ProcessResponse:
    return await process(request)

@app.post("/run", response_model=ProcessResponse, response_model_exclude_none=True)
